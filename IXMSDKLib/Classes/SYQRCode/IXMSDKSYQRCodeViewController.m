...
@interface IXMSDKSYQRCodeViewController () <AVCaptureMetadataOutputObjectsDelegate>{
    IXMConfig *config;
...
@implementation IXMSDKSYQRCodeViewController
...
- (void)setOverlayPickerView
{
    ...
    labIntroudction.frame = CGRectMake(20, CGRectGetMinY(downView.frame) + 25, self.view.frame.size.width-40, 60);
    labIntroudction.textAlignment = NSTextAlignmentCenter;
    labIntroudction.font = [UIFont boldSystemFontOfSize:13.0];
    labIntroudction.textColor = [UIColor whiteColor];
    labIntroudction.text = @"将i厦门网页版登录二维码置于框内,扫描后可自动登录网页版";
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (metadataObjects.count > 0)
    {
        ...
        {
            if ([obj.stringValue containsString:@"QRCODELOGIN-WEB"])
            {
                if (self.IXMSDKSYQRCodeSuncessBlock) {
                    self.IXMSDKSYQRCodeSuncessBlock(self,obj.stringValue);
                }
            }
            else
            {
                if (self.IXMSDKSYQRCodeFailBlock) {
                    self.IXMSDKSYQRCodeFailBlock(self);
                }
            }
        }
        else
...
- (void)cancleSYQRCodeReading
{
    ...
    if (self.IXMSDKSYQRCodeCancleBlock)
    {
        self.IXMSDKSYQRCodeCancleBlock(self);
    }
